...
return this.each(function(){
	...
	if(_box.is(':visible')){
		if(_box.children('.scroll-content').length == 0){
			...
			/*--- init part ---*/
			var scrollBar = jQuery('<div class="scroll-bar"><div class="scroll-up"></div><div class="scroll-line" style="display:none"><div class="scroll-slider"><div class="scroll-slider-inner"><div class="scroll-slider-inner-stp"></div></div></div></div><div class="scroll-down"></div></div>');
			...
			_box.bind('mousewheel', function(event, delta, deltaX, deltaY){
				if(_f){
					//console.info("_top === "+_top + " delta = " + delta + " _s1 " + _s1);
					_top -=deltaY*_s1;
					//if(_top < 0) _top = _top + (_top * -1);
					//console.info("after _top = " + _top);
					scrollCont();
					if((_top > 0) && (_top+slider_h < scrollSliderH.height())) return false;
				}
			});
			scrollContent.mouseover(function() {
				_box.find('.scroll-line').css('display', 'block');
			}).mouseout(function(){
				_box.find('.scroll-line').css('display', 'none');
			});
			function scrollCont(){
				if(_top < 0) _top = 0;
				else if(_top+slider_h > scrollSliderH.height()) _top = scrollSliderH.height() - slider_h;
				//console.info("scrollCont _top = " + _top + " slider_f = " + slider_f);
