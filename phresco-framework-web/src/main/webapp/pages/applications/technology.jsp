...
// 		projectCode = selectedInfo.getCode();
		...
		if (CollectionUtils.isNotEmpty(selectedDatabases)) {
			for (DownloadInfo selectedDatabase : selectedDatabases) {
	...
	$(document).ready(function() {
		...
		<%
			...
			if (selectedServers != null) {
				for (DownloadInfo server : selectedServers) {
					String csvVersion = "";
					name = server.getName();
					//TODO:Need to handle
...
// 					}
		...
		<%
			...
			if (selectedDatabases != null) {
				for (DownloadInfo database : selectedDatabases) {
					String csvVersion = "";
		...
		if (pilotProject != undefined && pilotProject != "" && alreadyConstructed == "") { //To show the pilot project configurations
			...
			<%
				if (CollectionUtils.isNotEmpty(pilotProjectServers)) {
					...
					for (DownloadInfo pilotProjectServer : pilotProjectServers) {
						String csvVersion = "";
			...
			<%
				if (CollectionUtils.isNotEmpty(pilotProjectDbs)) {
					...
					for (DownloadInfo pilotProjectDb : pilotProjectDbs) {
						String csvVersion = "";
