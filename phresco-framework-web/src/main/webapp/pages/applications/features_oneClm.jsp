...
<%
    ...
    List<ArtifactGroup> modules = (List<ArtifactGroup>) request.getAttribute(FrameworkConstants.REQ_FEATURES_LEFT_MODULES);
...
<form class="app_features_form" id="formFeatures" autocomplete="off">
	<div class="clearfix" style="margin: 5px 0 0 5px;">
		<label for="xlInput" style="width: auto;"><s:text
				name="label.select.pilot.project" />
		</label>
		...
		<div class="input">
			...
			%>
			<input type="text" class="medium" id="selectedPilotProject"
				name="selectedPilotProject" disabled="disabled"
				value="<%=pilotProject%>">
		</div>
	...
	<div class="featuresScrollDiv">
		...
		<div class="alert-message block-message warning">
			<center>
				<label class="errorMsgLabel"><s:text
						name="label.feature.not.avail" />
				</label>
			</center>
		</div>
		...
		<div class="tblheader">
			<table class="zebra-striped">
				<tr>
					<th class="editFeatures_th1"><input type="checkbox"
						value="AllJsLibs" id="checkAllJsLibs" name="<%=moduleType%>"
						onchange="selectAllChkBoxClk('<%=moduleType%>', this)">
					</th>
     ...
     for (ArtifactGroup module : modules) {
 									...
 									if (moduleHdr.equals(FrameworkConstants.REQ_JS_LIBS)) {
 										if (MapUtils.isNotEmpty(appInfoJsLibs) && StringUtils.isNotEmpty(appInfoJsLibs.get(module.getId()))) {
 											pilotVersion = appInfoJsLibs.get(module.getId());
 											checkedStr = "checked";
 										...
 										if (MapUtils.isNotEmpty(appInfoModules) && StringUtils.isNotEmpty(appInfoModules.get(module.getId()))) {
 											pilotVersion = appInfoModules.get(module.getId());
 											checkedStr = "checked";
 											disabledStr = "disabled";
 										}
 ...
 %> <span class="siteaccordion closereg"> <span> <%
     if (moduleHdr.equals(FrameworkConstants.REQ_JS_LIBS)) {
 %>
						<input type="checkbox"
						class="<%=FrameworkConstants.REQ_JSLIB_MODULE%>"
						name="<%=FrameworkConstants.REQ_FEATURES_JSLIBS%>"
						value="<%=module.getId()%>" <%=disabledStr%> <%=checkedStr%>
						id="<%=module.getId()%>checkBox"> <%
     } else if (moduleHdr.equals(FrameworkConstants.REQ_CUSTOM_FEATURES)) {
 %>
						<input type="checkbox"
						class="<%=FrameworkConstants.REQ_CUSTOM_MODULE%>"
						name="<%=FrameworkConstants.REQ_FEATURES_MODULES%>"
						value="<%=module.getId()%>" <%=disabledStr%> <%=checkedStr%>
						id="<%=module.getId()%>checkBox"> <%
     } else {
 %> <input
						type="checkbox" class="<%=FrameworkConstants.REQ_CORE_MODULE%>"
						name="<%=FrameworkConstants.REQ_FEATURES_MODULES%>"
						value="<%=module.getId()%>" <%=disabledStr%> <%=checkedStr%>
						id="<%=module.getId()%>checkBox"> <%
     }
 %> &nbsp;&nbsp;<%=module.getName()%>&nbsp;&nbsp;
						<p id="<%=module.getId()%>version"
							class="version versionDisplay_JSLib"></p> </span> </span>
							...
							<tbody>
								<%
								    String serverUrl = (String) request.getAttribute(FrameworkConstants.REQ_SERVER_URL);
																		    	...
																		    	List<ArtifactInfo> versions = module.getVersions();
								...
								%>
								<tr>
									<td class="editFeatures_td1"><input type="radio"
										name="<%=module.getId()%>"
										value="<%=moduleVersion.getVersion()%>" <%=disabledStr%>
										<%=checkedStr%>
									...
									<td class="editFeatures_td2">
										...
										%> <a
										href="#" name="ModuleDesc" descImage="<%=url%>"
										descrContent="<%=descContent%>" class="<%=module.getId()%>"
										id="<%=helpTextContent%>"><%=module.getName()%></a></td>
	...
	<div class="features_actions">
		<a id="previous" href="#" class="primary btn"><s:text
				name="label.previous" />
		</a>
		<%
		    if(StringUtils.isNotEmpty(fromPage)) {
		%>
		<input id="update" type="button" value="<s:text name="label.update"/>"
			class="primary btn createProject_btn">
		<%
		    } else {
		%>
		<input id="finish" type="button" value="<s:text name="label.finish"/>"
			class="primary btn createProject_btn">
		<%
		    }
		%>
		<input type="button" id="cancel" value="<s:text name="label.cancel"/>"
			class="primary btn">
	</div>

	<!-- Hidden Fields -->
	<input type="hidden" name="techId" value="<%=techId%>"> <input
		type="hidden" name="configServerNames"
		value="<%=configServerNames%>"> <input type="hidden"
		name="configDbNames" value="<%=configDbNames%>"> <input
		type="hidden" name="fromTab" value="features"> <input
		type="hidden" name="customerId" value="<%=customerId%>"> <input
		type="hidden" name="fromPage" value="<%=fromPage%>"> <input
		type="hidden" name="projectCode" value="<%=projectCode%>">

</form>

<!-- Tool tip div -->
<div class="twipsy bootstrap-right" id="toolTipDiv"
	style="display: none;">
...
<script type="text/javascript">
	...
	'); //this is for changing the sub-tab.
