	...
	<div class="modal-body perModalBody">
		<fieldset class="popup-fieldset fieldsetBottom perFieldSet">
            ...
            <div class="clearfix perClearFix">
            	<label class="xlInput popup-label perShowSettingLbl"></label>
            	<label  class="xlInput popup-label perSelectedTypeTitle">
            	<span class="red">*</span>
            	<s:text name="label.environment"/></label>
            	<label class="xlInput popup-label perfConfName" id="selectedTypeTitle">
            	<span class="red">*</span>
            	<s:text name="label.server"/></label>
            </div>
            
            <div class="clearfix perClearFix">
            	<label class="xlInput popup-label perShowSettingLbl">
            	...
            	<label class="xlInput popup-label perSelectBox">
            	...
            	<label class="xlInput popup-label perfConfName">
            		<input type="text" id="Server" name="Server" class="xlarge" style="width: 149px;" disabled="disabled">
					<select id="WebService" name="WebService" class="xlarge" style="width: 149px;display: none;"></select>
                   	<select id="Database" name="Database" class="xlarge" style="width: 149px;display: none;"></select>
            	</label>
            </div>
				...
				<table>
					<tr>
						<td style="width: 20%; border-bottom: none;">
							<label class="xlInput popup-label perTestName" style="width: 208px">
								<span class="red">*</span>
				...
				<div class="perContentDiv">
					<div class="screenUrlsContainer perScreenUrlContain" id="screenUrls">
						<fieldset class="popup-fieldset perContentFieldsetDiv">
							<label class="xlInput popup-label algnLeft perScreenNameTxt">
							...
							<div id="postData1" style="display:none;">
								...
								<div class="input padTop"  style="margin-left: 133px;">
									<textarea title="json data" class="screenUrl perTxtArea env-desc" id="contextPost1" 
										name="contextPostData" rows="1"></textarea>
								</div>
			                 </div>
				         </fieldset>
		             </div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#testName").autocomplete({
			select:function(event, ui) {
				var params = "selectedTestName=";
				params = params.concat(ui.item.value);
				performAction('getPerfTestJSONData', $('#formJmeterPerform'), '', true, params);
			}
		});  
		...
		$("#addUrl").click(function () {
			...
			newTextBoxDiv.html("<fieldset class='popup-fieldset perContentFieldsetDiv'><label class='xlInput popup-label algnLeft perScreenNameTxt'><input type='textbox' name='name' id='" + nameId + "' value='' class='screenName' title='Name' maxlength='20'>" +
					...
					"</select>&nbsp;&nbsp;&nbsp;&nbsp;<select id='" + encodingId + "' name='encodingType' class='perEncodingField'><option value='UTF-8'>UTF-8</option><option value='UTF-16'>UTF-16</option></select> </div><div id='" + postDivId + "' style='display:none;'><label class='xlInput popup-label algnLeft perScreenNameTxt'><span class='red'>*</span> Request Body</label> <div class='input padTop'  style='margin-left: 133px;'><textarea title='json data' class='screenUrl perTxtArea env-desc' id='" + contextPostId + "' name='contextPostData' rows='1'></textarea></div></div></fieldset>");
			// root element
		...
		$("#addDbUrl").click(function () {
			...
			var qStmt = "queryStatement" +counter;
		 	...
		 	newDBDiv.html("<fieldset class='popup-fieldset perContentFieldsetDiv'><label class='xlInput popup-label algnLeft perScreenNameTxt'><input type='textbox' name='dbPerName' id='" + qName + "' value='' class='screenName'title='Name' maxlength='20'>" +
	...
	function successEnvValidation(data) {
		if (data.hasError == true) {
			...
			if (validator) {
				...
				readerHandlerSubmit('perTest', '<%= projectCode %>', '<%= FrameworkConstants.PERFORMACE %>', $('#formJmeterPerform'));
			}
		}
	}
	...
	function loadTestFiles() {
    	performAction('tstResultFiles', $('#formJmeterPerform'), '', true);
    }
	...
	function fillJSONData(data) {
		console.info("inside fillJSONData:::" + data);
	...
	function addContextUrl(name, context, contextType, encodingType, contextPostData) {
		var nameId = "name" + counter;
	 	...
	 	var newTextBoxDiv = $(document.createElement('div')).attr("id", 'urlGroup' + counter);
	 	newTextBoxDiv.html("<fieldset class='popup-fieldset perContentFieldsetDiv'><label class='xlInput popup-label algnLeft perScreenNameTxt'><input type='textbox' name='name' id='" + nameId + "' value='" + name + "' class='screenName'>" +
				...
				"</select>&nbsp;&nbsp;&nbsp;&nbsp;<select id='" + encodingId + "' name='encodingType' class='perEncodingField'><option value='UTF-8' "+ utf8option +">UTF-8</option><option value='UTF-16' "+ utf16option +">UTF-16</option></select> </div><div id='" + postDivId + "' style='display:"+ postDivDisp +"'><label class='xlInput popup-label algnLeft perScreenNameTxt'>Request Body</label><div class='input padTop'  style='margin-left: 133px;'><textarea title='json data' class='screenUrl perTxtArea' id='" + contextPostId + "' name='contextPostData' rows='1'>"+ contextPostData +"</textarea></div></div></fieldset>");
		// root element
		newTextBoxDiv.appendTo("#screenUrls");
		counter++;
	}
	...
	function adddbContextUrl(dbName,queryType,query) {
		...
		var qStmt = "queryStatement" +counter;
	 	var newDBDiv = $(document.createElement('div')).attr("id", 'urlGroup' + counter);
	 	newDBDiv.html("<fieldset class='popup-fieldset perContentFieldsetDiv'><label class='xlInput popup-label algnLeft perScreenNameTxt'><input type='textbox' name='dbPerName' id='" + qName + "' value='"+ dbName +"' class='screenName'title='Name' maxlength='20'>" +
