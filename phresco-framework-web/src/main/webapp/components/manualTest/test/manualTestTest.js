...
//		 list testcases
		runListManualTestcase: function (moduleName) {
			...
			asyncTest("List Testcases", function() {
				...
				$.mockjax({
					  url : commonVariables.webserviceurl + commonVariables.manual + '/testcaseValidation?testSuiteName=Downloads&appDirName=test&testCaseId=sampletestcase',
					  type: "POST",
					  dataType: "json",
					  contentType: "application/json",
					  status: 200,
					  response : function() {
						  this.responseText = JSON.stringify({"message":null,"exception":null,"responseCode":"PHRQ400007","data":true,"status":"success"});
					  }
					});
		...
		runAddTestcase: function (moduleName) {
			...
			asyncTest("Adding testcases to a testsuite", function() {
				...
				$.mockjax({
					  url : commonVariables.webserviceurl + commonVariables.manual + '/testcaseValidation?testSuiteName=Downloads&appDirName=test&testCaseId=sampletestcase&&userId=jagadeesh_r',
					  type: "POST",
					  dataType: "json",
					  contentType: "application/json",
					  status: 200,
