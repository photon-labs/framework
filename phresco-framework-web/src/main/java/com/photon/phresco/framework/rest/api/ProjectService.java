...
public class ProjectService extends RestBase implements FrameworkConstants {
	...
	public Response list(@QueryParam("customerId") String customerId) {
...
//            }
	...
	public Response editProject(@QueryParam("projectId") String projectId, @QueryParam("customerId") String customerId ) {
		...
		try {
			ProjectManager projectManager = PhrescoFrameworkFactory.getProjectManager();
			projectInfo = projectManager.getProject(projectId, customerId);	
			ResponseInfo<ProjectInfo> finalOutput = responseDataEvaluation(responseData, null, "Project edited Successfully", projectInfo);
			return Response.status(Status.OK).entity(finalOutput).header("Access-Control-Allow-Origin", "*").build();
		} catch (PhrescoException e) {
	...
	public Response updateProject(ProjectInfo projectinfo, @QueryParam("userId") String userId) {
		...
		try {
			...
			ResponseInfo<ProjectInfo> finalOutput = responseDataEvaluation(responseData, null, "Project updated Successfully", projectInfo);
			return Response.status(Status.OK).entity(finalOutput).header("Access-Control-Allow-Origin", "*").build();
		} catch (PhrescoException e) {
	...
	public Response updateApplication( @QueryParam("oldAppDirName") String oldAppDirName , ApplicationInfo appInfo, @QueryParam("userId") String userId, @QueryParam("customerId") String customerId) {
		...
		try {
			ServiceManager serviceManager = CONTEXT_MANAGER_MAP.get(userId);
			if(serviceManager == null) {
				ResponseInfo<ApplicationInfo> finalOutput = responseDataEvaluation(responseData, null, "UnAuthorized User", null);
				return Response.status(Status.BAD_REQUEST).entity(finalOutput).header("Access-Control-Allow-Origin", "*").build();
			}
		...
		try {
			...
			for (ApplicationInfo applicationInfo : appInfos) {
				if(applicationInfo.getAppDirName().equals(appDirName)) {
					ResponseInfo<ProjectInfo> finalOutput = responseDataEvaluation(responseData, null, "Application edited Successfully", projectInfo);
					return Response.status(Status.OK).entity(finalOutput).header("Access-Control-Allow-Origin", "*").build();
				}
			}
		} catch (FileNotFoundException e) {
			ResponseInfo<ProjectInfo> finalOutput = responseDataEvaluation(responseData, e, "Application edit Failed", null);
			...
			if (CollectionUtils.isNotEmpty(appDirnames)) {
				for (String appDirName : appDirnames) {
					...
					if (file.exists()) {
						...
						if(connectionAlive) {
							ResponseInfo finalOutput = responseDataEvaluation(responseData, null, "Unable to delete the Application", null);
							return Response.status(Status.BAD_REQUEST).entity(finalOutput).header("Access-Control-Allow-Origin", "*").build();
						}
					}
				}
